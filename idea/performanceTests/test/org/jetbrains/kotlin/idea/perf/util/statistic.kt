...
class Statistic {
    ...
    class ApplicationScope(val rootDisposable: Disposable, val stats: StatsScope) {
        ...
        fun gradleProject(path: String, block: ProjectScope.() -> Unit) =
            ProjectScope(ProjectScopeConfig(path, ProjectOpenAction.GRADLE_PROJECT), this).block()

        companion object {
            private fun initApp(rootDisposable: Disposable): IdeaTestApplication? {
